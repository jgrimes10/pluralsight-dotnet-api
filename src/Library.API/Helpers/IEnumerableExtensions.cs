...
namespace Library.API.Helpers
{
    public static class IEnumerableExtensions
    {
            ...
            string fields)
        {
            ...
            {
                // on the public properties that match the fields should be
                // in the ExpandoObject

                // the fields are separated by "," so split it
                var fieldsAfterSplit = fields.Split(',');

                foreach (var field in fieldsAfterSplit)
                {
                    // trim each field as it might contain leading or trailing
                    // spaces; can't trim the var in foreach, so use another var
